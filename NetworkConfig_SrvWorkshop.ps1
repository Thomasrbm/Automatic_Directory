...
#               - Carte Internal : IP fixe sur le reseau interne workshop (pas de passerelle)
...
$BridgedIP  = Get-Input "IP fixe Bridged (ex: 10.12.200.164)" "IP Bridged" "10.12.200.164"
...
$AdminIP    = Get-Input "IP Bridged du SRV-ADMIN (DNS principal, ex: 10.12.200.163)" "IP SRV-ADMIN" "10.12.200.163"
...
Set-DnsClientServerAddress -InterfaceAlias $BridgedNic -ServerAddresses $AdminIP
Write-Host "Bridged configure : $BridgedIP/$BridgedPfx  GW $BridgedGW  DNS $AdminIP" -ForegroundColor Green

# --- CARTE INTERNAL (reseau workshop interne) ---
Write-Host "`n[2/2] Configuration carte Internal (reseau workshop)..." -ForegroundColor Yellow
$InternalNic = Get-Input "Nom EXACT de la carte Internal (voir liste ci-dessus)" "Carte Internal" "Ethernet 2"
$InternalIP  = Get-Input "IP fixe Internal (ex: 192.168.20.1)" "IP Internal" "192.168.20.1"
...
Set-DnsClientServerAddress -InterfaceAlias $InternalNic -ServerAddresses $AdminIP
